...
import { selectProgressBars, selectReviews } from "@/store/user/userSelectors";
import Logo from "@/components/elements/Logo/Logo";
import { Card, CardContent } from "@/components/ui/card";

const ProgressSection = () => {
  const reviewsData = useAppSelector(selectReviews);
  const progressBarsData = useAppSelector(selectProgressBars);
    ...
    <section className="section h-full">
      <div className="h-full flex flex-col justify-between mx-auto">
        ...
        <Dialog open={showPopup} onOpenChange={() => {}}>
          <DialogContent className="p-[2rem] bg-white">
            <DialogHeader>
              <DialogTitle className="text-[1.2rem] text-[#666666] font-semibold text-center">
                To move forward, specify
              </DialogTitle>
            </DialogHeader>
            <DialogDescription className="text-[1.8rem] font-bold text-center">
        ...
        <div className="w-full relative mt-auto mb-[4rem]">
          <motion.div
            key={currentReview}
